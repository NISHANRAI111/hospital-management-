        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'success') {
                    let html = '<table><tr><th>Name</th><th>Specialization</th><th>Experience</th><th>Action</th></tr>';
                    response.doctors.forEach(function(doc) {
                        html += `<tr><td>${doc.full_name}</td><td>${doc.specialization || 'N/A'}</td><td>${doc.years_of_experience || 'N/A'}</td><td><button class="delete-user" data-id="${doc.user_id}">Delete</button></td></tr>`;
                    });
                    html += '</table>';
                    $('#doctors-list').html(html);
                } else {
                    $('#doctors-list').html('<p>No doctors.</p>');
                }
            }
        });
        ...
        $(document).on('click', '.delete-user', function() {
            ...
            if (confirm('Delete this doctor?')) {
                $.ajax({
                    ...
                    type: 'POST',
                    data: { user_id: userId, role: 'Doctor' },
                    dataType: 'json',
                    success: function(response) {
